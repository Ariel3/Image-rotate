function imr = myImRotate(im, theta)
% imr = myImRotate(im, theta)
% This function rotates image im by angle degrees in a 
% counterclockwise direction around its center point.
% It might creates black borders.
...
for yr= 1:Yr
    for xr= 1:Xr
        ...
        if ( (y_u>=1)&&(y_u<=Y) && ...
            ...
            cg= im(y_d,x_d);
            dg= im(y_d,x_u);
            imr(yr,xr)= 0.25*ag + 0.25*bg + 0.25*cg + 0.25*dg;
        end
    end
